...
static int my_readdir(const char *path, void *buf, fuse_fill_dir_t filler,  off_t offset, struct fuse_file_info *fi)
{
    ...
    for(i = 0; i < MAX_FILES_PER_DIRECTORY; i++) {
        if(!(myFileSystem.directory.files[i].freeFile)) {
        	if(myFileSystem.directory.files[i].fileName[0] != "."){
...
int my_unlink(const char *filename) {
	...
	if (nodeIndexDir != -1) {
		...
		myFileSystem.superBlock.numOfFreeBlocks+=nodo->numBlocks;          // + numero de bloques del nodo libres
		updateBitmap(&myFileSystem);
		updateSuperBlock(&myFileSystem);
 	 
 		// lo liberamos del sistema
		nodo->numBlocks=0;
		nodo->fileSize=0;
		nodo->freeNode=true;

		updateNode(&myFileSystem,direccion,nodo);
	
		// liberamos el puntero al nodo
		free(nodo);
		myFileSystem.nodes[direccion]=NULL;    ///ahora esa direccion sera NULL porque ya no hay nada
	}
...
int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	char buffer[BLOCK_SIZE_BYTES];              // buffer de chars de tamaño de  bloque
	int bytesPorLeer = size, bytesLeidos = 0;

	while(bytesPorLeer) {                       // mientras tengamos bytes que leer
		int bloque = myFileSystem.nodes[fi->fh]->blocks[offset / BLOCK_SIZE_BYTES];
		int offBloque = offset % BLOCK_SIZE_BYTES;
		 
		// posicionamos el puntero del archivo en el bloque + offset
		lseek(myFileSystem.fdVirtualDisk, bloque * BLOCK_SIZE_BYTES, SEEK_SET);

        // para leer un bloque entero empezando en esa posición
		if (read(myFileSystem.fdVirtualDisk, &buffer, BLOCK_SIZE_BYTES) != 0) {
			// Todo lo leido de ese bloque lo escribimos en el buffer
			for(int i = offBloque; (i < BLOCK_SIZE_BYTES) && (bytesLeidos < size); i++) {
				buf[bytesLeidos] = buffer[i];
				offset++;
				bytesLeidos++;
				bytesPorLeer--;
			}
			
			bytesPorLeer-=offBloque;
		}
	}
	sync();
